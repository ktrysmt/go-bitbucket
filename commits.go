...
func (cm *Commits) GetCommits(cmo *CommitsOptions) interface{} {
	url := cm.c.requestUrl("/repositories/%s/%s/commits/%s", cmo.Owner, cmo.RepoSlug, cmo.Branchortag)
...
func (cm *Commits) GetCommit(cmo *CommitsOptions) interface{} {
	url := cm.c.requestUrl("/repositories/%s/%s/commit/%s", cmo.Owner, cmo.RepoSlug, cmo.Revision)
	return cm.c.execute("GET", url, "")
}

func (cm *Commits) GetCommitComments(cmo *CommitsOptions) interface{} {
	url := cm.c.requestUrl("/repositories/%s/%s/commit/%s/comments", cmo.Owner, cmo.RepoSlug, cmo.Revision)
	return cm.c.execute("DELETE", url, "")
}

func (cm *Commits) GetCommitComment(cmo *CommitsOptions) interface{} {
	url := cm.c.requestUrl("/repositories/%s/%s/commit/%s/comments/%s", cmo.Owner, cmo.RepoSlug, cmo.Revision, cmo.CommentID)
	return cm.c.execute("GET", url, "")
}

func (cm *Commits) GetCommitStatus(cmo *CommitsOptions, commitStatusKey string) interface{} {
	url := cm.c.requestUrl("/repositories/%s/%s/commit/%s/statuses/build/%s", cmo.Owner, cmo.RepoSlug, cmo.Revision, commitStatusKey)
	return cm.c.execute("GET", url, "")
}

func (cm *Commits) GiveApprove(cmo *CommitsOptions) interface{} {
	url := cm.c.requestUrl("/repositories/%s/%s/commit/%s/approve", cmo.Owner, cmo.RepoSlug, cmo.Revision)
